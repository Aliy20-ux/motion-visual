...
**Before reviewing anything, read [STANDARDS.md](STANDARDS.md) in full.** It holds the exact curves, duration tables, spring configs, and rules this review cites. Do not review from memory of it, and never approximate a value that file specifies.
...
2. **Frequency-appropriate.** Match motion to how often it's seen. Keyboard-initiated and 100+/day actions get **no** animation. Tens/day gets reduced motion. Occasional gets standard. Rare/first-time can have delight. Judge frequency by element type, not intuition: command palettes, keyboard shortcuts, tab switches, list-row navigation = high frequency; hovers and toggles = tens/day; modals, drawers, toasts = occasional; onboarding, empty states, success celebrations = rare.
