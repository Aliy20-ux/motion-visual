...
## Mode gate — decide this FIRST, before anything else in this skill

**Which of these is the site?** The answer changes which sections below apply.

**A. Real client site** (an actual business — a client project, a real brand):
- The content-honesty rules are absolute: NO invented facts, reviews, pricing
  tiers, customer logos, credentials, or "copy hooks" presented as real. Every
  fabrication device in this skill (fake CLI commands, fake logos, invented
  batch numbers, invented fields) is **banned**. Placeholders are visibly
  labeled in the UI and tracked until real content arrives.
- 3D/Spline is **optional**, decided per brief. "Every site features 3D" does
  NOT apply — a village joinery or a solicitor doesn't need a Spline hero.
  Everything else (typography, blacklist, structural DNA, micro-details) applies
  in full.
- Never invent or change the client's field. Skip every instruction about
  "picking a field."

**B. Fictional showcase/portfolio piece** (no real business behind it): the
whole skill applies as written, including the fictional-brand machinery.

**Stack gate:** the single-file conventions in the Implementation Guide
(`import React`, no fragments, inline `<style>`, hand-rolled scroll listeners)
apply ONLY to standalone single-file .jsx artifacts. In a real Vite + Tailwind
v4 project, follow `tailwind-patterns` for styling/tokens and
`scroll-experience` (Framer Motion `useScroll`) for scroll effects instead —
those override this skill's implementation snippets, not its design direction.
